...
export default async function GeofencesPage() {
  ...
  return (
    <div>
      ...
      {records.length === 0 ? (
        <div className="panel empty-state"><p>No geofences yet</p></div>
      ) : (
            ...
            <tbody>
              {records.map((r: any) => (
                <tr key={r.id}>
                  ...
                  <td>{r.shape}</td>
                  <td><span className={`badge ${r.active ? 'badge-green' : 'badge-gray'}`}>{r.active ? 'Yes' : 'No'}</span></td>
                  <td>{r.radius != null ? `${r.radius}m` : '—'}</td>
